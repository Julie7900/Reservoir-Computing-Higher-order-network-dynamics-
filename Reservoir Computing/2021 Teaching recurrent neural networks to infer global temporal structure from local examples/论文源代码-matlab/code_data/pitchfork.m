classdef pitchfork < handle
    % Defines a class for an N dimensional nonlinear reservoir
    properties
        x0              % Initial State: 1 x 1
        x               % Current State: 1 x 1
        delT
        parms
    end
    ...
    methods
        % Constructor
        function obj = pitchfork(x0, delT, parms)
            obj.x0 = x0;
            obj.x = x0;
            obj.delT = delT;
            obj.parms = parms;
        ...
        function X = propagate(o,n)
            ...
            X = zeros([1,n,4]); 
            X(:,1,1) = o.x;
            dxdt = @(x) [o.parms(1)*x + o.parms(2)*x.^3];
            ...
            for i = 2:n
                ...
                X(:,i-1,2:4) = reshape([o.x+k1/2, o.x+k2/2, o.x+k3], [1 1 3]);
                o.x = X(:,i,1);
            end
            fprintf('\n');
        end
